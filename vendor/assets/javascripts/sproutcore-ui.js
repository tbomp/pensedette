...
UI.ContainerViewController = UI.ViewController.extend({

  viewsBinding: 'view.childViews',

  currentViewBinding: 'view.currentView',

  currentViewIdBinding: 'currentView.elementId',
   ...
   */
  destroy: function() {
    get(this, 'views').forEach(function(view) {
 ...
 */
UI.ContainerView = SC.ContainerView.extend(UI.LayoutSupport, UI.ViewControllerSupport, {
  classNames: ['ui-container'],
   ...
   */
  addView: function(view) {
    view = this.createChildView(view, {isVisible: false});
   ...
   */
  removeView: function(view) {
    if (get(this, 'currentView') === view) {
      set(this, 'currentView', null);
    }
    view.destroy();
  },

  animate: function(animationName, fromView, toView) {
    var animation = UI.ContainerView.animations[animationName];
    if (!animation) {
...
UI.ContainerView.animations = {
  ...
  flipLeft: {
    duration: 1000,
    setup: {webkitBackfaceVisibility: 'hidden'},
    'in': {
      '0%': {
...
/**
  @extends UI.ContainerView
 */
UI.NavigationView = UI.ContainerView.extend({
  classNames: ['ui-navigation'],

  itemViewClass: UI.View,
...
UI.NavigationViewController = UI.ContainerViewController.extend({
...
/**
  @extends UI.ContainerView
 */
UI.TabView = UI.ContainerView.extend({
  classNames: ['ui-tab'],
...
UI.TabViewController = UI.ContainerViewController.extend({
   ...
   *
   */
  selectView: function(view, animation) {
    SC.run.schedule('sync', this, function() {
      if (typeof view === 'string') {
        view = SC.View.views[view];
      }
  ...
  didInsertView: function() {
    ...
    views.setEach('isVisible', false);
    this.selectView(get(views, 'firstObject'));
    // get(this, 'views').forEach(function(view, i) {
    //   if (i === 0) {
...
UI.Button = SC.Button.extend(UI.ButtonSupport, {
  ...
  */
  ...
  //   options.data.buffer.push(context.get('formattedTitle'));
  // }
});
...
*/
SC.Form = SC.View.extend(SC.TargetActionSupport, {
  tagName: 'form',
  classNames: ['sc-form'],
...
UI.Select = SC.CollectionView.extend(UI.SelectableSupport, {
  tagName: 'select',
  classNames: ['sc-select', 'ui-select'],
...
SC.Handlebars.registerHelper('ui', function(path, options) {
  ...
  if (SC.View.detect(controller)) {
    return SC.Handlebars.ViewHelper.helper(this, controller, options);
  }
  var viewClass = get(controller, 'view'),
  ...
  if (SC.View.detect(viewClass)) {
    ...
    return SC.Handlebars.ViewHelper.helper(this, viewClass, options);
  } else if (viewClass && viewClass.isView) {
    viewClass.rerender();
  } else {
